...
Les graphiques sont rendus avec **Chart.js** : donuts pour les trois KPIs globaux et la complétude des données, camembert pour la criticité, barres horizontales empilées pour l'hébergement et radar pour la couverture d'interfaces. Les couleurs suivent la convention bleu (alignement), orange (mutualisation), rouge (critique) et vert (standard). Les pourcentages sont recalculés dès que vous modifiez les filtres puis rouvrez le rapport. Pour fonctionner sans accès Internet, la bibliothèque est fournie localement dans `public/vendor/chart.umd.js`.
