...
Les graphiques sont rendus avec **ECharts** : donuts pour les trois KPIs globaux et la complétude des données, camembert pour la criticité, barres horizontales empilées pour l'hébergement et radar pour la couverture d'interfaces. Les couleurs suivent la convention bleu (alignement), orange (mutualisation), rouge (critique) et vert (standard). Les pourcentages sont recalculés dès que vous modifiez les filtres puis rouvrez le rapport. ECharts est livré directement dans `public/vendor/echarts.min.js`, ce qui évite toute erreur de type MIME.
Le script est inclus via un chemin relatif `vendor/echarts.min.js` pour fonctionner même lorsque la page est ouverte localement.
