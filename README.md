...
Les graphiques sont rendus avec **Chart.js** : donuts pour les trois KPIs globaux et la complétude des données, camembert pour la criticité, barres horizontales empilées pour l'hébergement et radar pour la couverture d'interfaces. Les couleurs suivent la convention bleu (alignement), orange (mutualisation), rouge (critique) et vert (standard). Les pourcentages sont recalculés dès que vous modifiez les filtres puis rouvrez le rapport. Chart.js est installé via npm et servi depuis `node_modules` afin de fonctionner même hors ligne.
